...
class EOSKey:
    ...
    def sign(self, digest):
        ...
        # logging.info("unhexlify 耗时========= %s", self.now_time() - strm)
        ...
        while 1:
            ...
            # logging.info("ecdsa.rfc6979.generate_k 耗时========= %s", self.now_time() - strm)

            # sign the message
            sigder = self._sk.sign_digest(digest, sigencode=ecdsa.util.sigencode_der, k=k)

            # logging.info("sign_digest 耗时========= %s", self.now_time() - strm)

            # reformat sig
            r, s = ecdsa.util.sigdecode_der(sigder, self._sk.curve.generator.order())
            # logging.info("ecdsa.util.sigdecode_der 耗时========= %s", self.now_time() - strm)

            sig = ecdsa.util.sigencode_string(r, s, self._sk.curve.generator.order())
            # logging.info("ecdsa.util.sigencode_string 耗时========= %s", self.now_time() - strm)
            ...
            if lenR is 32 and lenS is 32:
                ...
                i += 27
                break
            # logging.info("if end 耗时========= %s", self.now_time() - strm)

        # pack
        sigstr = struct.pack('<B', i) + sig
        # logging.info("struct.pack 耗时========= %s", self.now_time() - strm)

        encodes = self._check_encode(hexlify(sigstr), 'K1').decode()
        # logging.info("encodes 耗时========= %s", self.now_time() - strm)

        # encode
        return 'SIG_K1_' + encodes
